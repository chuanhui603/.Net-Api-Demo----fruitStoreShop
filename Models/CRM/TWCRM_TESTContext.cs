// <auto-generated> This file has been auto generated by EF Core Power Tools. </auto-generated>
#nullable enable
using System;
using System.Collections.Generic;
using Microsoft.EntityFrameworkCore;

namespace 水水水果API.Models.CRM;

public partial class TWCRM_TESTContext : DbContext
{
    public TWCRM_TESTContext(DbContextOptions<TWCRM_TESTContext> options)
        : base(options)
    {
    }

    public virtual DbSet<Customer> Customers { get; set; }

    public virtual DbSet<CustomerGroup> CustomerGroups { get; set; }

    public virtual DbSet<Member> Members { get; set; }

    public virtual DbSet<MemberTier> MemberTiers { get; set; }

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<Customer>(entity =>
        {
            entity.HasKey(e => e.Id).HasName("PK__Customer__3214EC0710355252");

            entity.Property(e => e.Id)
                .HasDefaultValueSql("(NEXT VALUE FOR [Customer_Seq])")
                .HasAnnotation("Relational:DefaultConstraintName", "DF_Customer_Id");
        });

        modelBuilder.Entity<CustomerGroup>(entity =>
        {
            entity.HasKey(e => e.Id).HasName("PK__Customer__3214EC075ADB9FE7");

            entity.Property(e => e.IsActive).HasDefaultValue(true);
        });

        modelBuilder.Entity<Member>(entity =>
        {
            entity.HasKey(e => e.Id).HasName("PK__Member__3214EC0735840822");

            entity.Property(e => e.Id)
                .HasDefaultValueSql("(NEXT VALUE FOR [Member_Seq])")
                .HasAnnotation("Relational:DefaultConstraintName", "DF_Member_Id");
            entity.Property(e => e.IsActive).HasDefaultValue(true);

            entity.HasOne(d => d.Customer).WithMany(p => p.Members)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_Member_Customer");

            entity.HasOne(d => d.MemberTier).WithMany(p => p.Members).HasConstraintName("FK_Member_MemberTier");
        });

        modelBuilder.Entity<MemberTier>(entity =>
        {
            entity.HasKey(e => e.Id).HasName("PK__MemberTi__3214EC07BFE4C17B");

            entity.Property(e => e.Id)
                .HasDefaultValueSql("(NEXT VALUE FOR [MemberTier_Seq])")
                .HasAnnotation("Relational:DefaultConstraintName", "DF_MemberTier_Id");
            entity.Property(e => e.IsActive).HasDefaultValue(true);
        });
        modelBuilder.HasSequence("Customer_Seq");
        modelBuilder.HasSequence("Member_Seq");
        modelBuilder.HasSequence("MemberTier_Seq");

        OnModelCreatingPartial(modelBuilder);
    }

    partial void OnModelCreatingPartial(ModelBuilder modelBuilder);
}